...
const withDynamicAppIcon: ConfigPlugin<string[] | IconSet | void> = (
  config,
  props = {}
) => {
  const _props = props || {};
...
interface ManifestConfig {
  $: {
    ...
    "android:name": string;
    "androiid:permssion"?: string;
    "android:targetActivity": string;
  };
  "intent-filter": ManifestIntentFilter[];
}

interface IconManifestApplication extends ManifestApplication {
  "activity-alias": ManifestConfig[];
}
...
const withIconAndroidManifest: ConfigPlugin<Props> = (config, { icons }) => {
  return withAndroidManifest(config, (config) => {
    ...
    function addIconActivityAlias(config: ManifestConfig[]): ManifestConfig[] {
      return [
        ...config,
